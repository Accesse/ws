...
var util = require('util')
  ...
  , Options = require('ws/node_modules/options')
...
function WebSocketServer(options, callback) {
  ...
  if (options.isDefinedAndNonNull('port')) {
    ...
    this._closeServer = function() 	{ 	if (self._server)	{
									    	self._server.close(); 
									    }
    								};
  }
...
/**
 * Broadcast to all Clients of Webserver
 */
  ...
  var completeHybiUpgrade2 = function(protocol) {
    ...
    if (self.options.clientTracking) {
      ...
      client.on('close', function() {
        ...
        if (index !== -1) {
        	//faster?
	    	self.clients[index]=self.clients[self.clients.length-1];
	    	self.clients.pop();
    ...
    var completeHandshake = function(nonce, rest) {
      ...
      try {
        ...
        socket.write(handshakeBuffer, 'binary', function(err) {
          if (err) {
        	  return; // do not create client if an error happens
          }
